/*
  ...
  Stati riconosciuti per badge colorato:
  Disponibile, In trattativa, Esaurito
...
function renderProducts() {
  ...
  state.filteredProducts.forEach((product) => {
    ...
    const imageMarkup = product.image
      ? `<img class="product-image" src="${escapeAttribute(product.image)}" alt="${escapeAttribute(product.name)}" loading="lazy" decoding="async" width="512" height="512" onerror="this.remove();" />`
      : "";
    ...
    card.innerHTML = `
      ...
      <div class="product-content">
        ...
        <div class="product-meta">
          ...
          ${product.status ? `<span class="badge status-badge ${statusClass}">${escapeHtml(product.status)}</span>` : ""}
        </div>
...
function renderCart() {
  ...
  if (!items.length) {
    ...
    items.forEach((product) => {
      ...
      row.innerHTML = `
        ${product.image ? `<img class="cart-thumb" src="${escapeAttribute(product.image)}" alt="${escapeAttribute(product.name)}" loading="lazy" decoding="async" width="58" height="58" onerror="this.remove();" />` : `<div class="cart-thumb"></div>`}
        <div>
